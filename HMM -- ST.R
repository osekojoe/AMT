...
theme_minimal_adjstd <- function(...) {
    theme_minimal() +
        theme(
            plot.title = element_text(color = "gray0",  face = "bold", hjust = 0.5),
            axis.line = element_line(linetype = "solid"),
            axis.text.x = element_text(color = "gray0"),
            axis.text.y = element_text(color = "gray0"),
            axis.title.x = element_text(color = "gray0"),
            axis.title.y = element_text(color = "gray0"),
...
(acf_bs <- ggAcf(broadstreet$percocc, ci = NULL, color = "gray20") + theme_minimal_adjstd() +
    labs(x = "Lag", y = "ACF", title = "Autocorrelation function: Broad Street"))

(acf_ss <- ggAcf(shopping$percocc, ci = NULL, color = "gray20") + theme_minimal_adjstd() +
    labs(x = "Lag", y = "ACF", title = "Autocorrelation function: Shopping"))
...
(hist_obs_bs <- broadstreet %>% ggplot(aes(x = percocc)) +
        ...
        labs(x = "Relative occupancy", y = NULL, title = "Histogram of observed data: Broad street"))

(hist_obs_ss <- shopping %>% ggplot(aes(x = percocc)) +
        geom_histogram(color = "white", fill = "gray60", show.legend = F, binwidth = 0.05) +
        scale_fill_brewer(palette = "Set1") + 
        theme_minimal_adjstd() +
        labs(x = "Relative occupancy", y = NULL, title = "Histogram of observed data: Shopping"))

png("Figures_ak/hist_obs_ss.png", units="in", width = 9, height = 3.5, res = 300)
hist_obs_ss
dev.off()
...
(ts_bs <- broadstreet %>% ggplot(aes(x = lastupdated, y = percocc)) +
    ...
    theme_minimal_adjstd() +
    labs(y = "Relative occupancy", x = "Instance", y = NULL, title = "Time series plot: Broad street"))
...
(ts_ss <- shopping %>% ggplot(aes(x = lastupdated, y = percocc)) +
        ...
        # facet_wrap(~systemcodenumber, ncol = 2, scales = "free") +
        theme_minimal_adjstd() +
        labs(y = "Relative occupancy", x = "Instance", y = NULL, title = "Time series plot: Shopping"))
...
(zoomin_ts_ss <- zoomin_ts2 %>%
    ...
    theme(axis.text.x = element_text(size = 6)) +
    theme(axis.text.x = element_text(hjust = 0.3)) +
    labs(title = "Shopping (first 15 days)") +
    scale_x_datetime(
        breaks = custom_breaks, 
        labels = custom_labels, 
        expand = c(0,0)
...
png("Figures_ak/zoomin_ts_ss.png", units="in", width = 9, height = 3.5, res = 300)
zoomin_ts_ss
dev.off()
####################################################################################################
####################################################################################################
...
(state_pred_bs_15 <- broadstreet %>% mutate(viterbi = Viterbi(hmmbs15_fit)) %>%
        ...
        labs(x = "Time", y = "Occupancy proportion", color = "States"))
...
png("Figures_ak/hmmbs15_diagns.png", units="in", width = 9, height = 5, res = 300)
